...
class Board(object):
        ...
        for group in self.groups:
            ...
            for tile in group:
                if tile.value in CHOICES:
                    ...
                    log.debug(f"Duplicate {tile} in {group}")
        return True
    ...
    def naked_single(self) -> bool:
        ...
        """
        #first loop checks symbols in group
        progress = False
        for group in self.groups:
            sym_set = set()
            for tile in group:
                if tile.value not in UNKNOWN:
                    if tile.value not in sym_set:
                        sym_set.add(tile.value)
        #second loop removes candidate      
            for tile in group:
                if tile.value in UNKNOWN:
                    if tile.remove_candidates(sym_set):
                        progress = True
        return progress
    
    def hidden_single(self):
        """If unknown tiles still have at lest 2 candidate values but one tile has
        candidate value 3 it must hold 3 bc no other place to put it
        """
        #first loop checks symbols in group
        progress = False
        for group in self.groups:
        ...
        for group in self.groups:
            for tile in group:
                if tile.value == UNKNOWN:
                    if len(tile.candidates) < min_row:
                        min_row = len(tile.candidates)
                        pos = tile
        return pos
