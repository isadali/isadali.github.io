...
# The field bit positions/ unsigned ints
...
offset_field = BitField(0, 9) #signed int
...
def decode(word: int) -> Instruction:
    ...
    o = OpCode(op_field.extract(word))
    con_field = CondFlag(cond_field.extract(word))
    reg_tar_field = reg_target_field.extract(word)
    reg1_field = reg_src1_field.extract(word)
    reg2_field = reg_src2_field.extract(word)
    off_field = offset_field.extract_signed(word)
    return Instruction(o, con_field, reg_tar_field, reg1_field, reg2_field, off_field)
