...
class BitField(object):
    ...
    def __init__(self, from_bit: int, to_bit: int) -> None: 
        ...
        self.to_bit = to_bit
        #actual length of binary 
        width = abs(from_bit - (to_bit + 1))
        
        #numerical val of binary
        self.mask = 0
    ...
    def extract_signed(self, word: int) -> int:
        ...
        #calling self to use the method extract and word is method
        unsigned_int = self.extract(word)
        return (sign_extend(unsigned_int, (self.to_bit - self.from_bit + 1)))
