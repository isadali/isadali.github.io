...
class Read(Expr):
    ...
    def gen(self, context: Context, target: str):
        """We print by loading to the memory-mapped address 510"""
...
class If(Control):
    ...
    def gen(self, context: Context, target: str):
        ...
        _else = context.new_label("else")
        _fi = context.new_label("fi")
        self.cond.condjump(context, target, _else, jump_cond=False)
        self.thenpart.gen(context, target)
        context.add_line(f"   JUMP  {_fi}")
        context.add_line(f"{_else}:")
        self.elsepart.gen(context, target)
        context.add_line(f"{_fi}:")
